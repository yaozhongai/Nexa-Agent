...
用法::
    ...
    # 运行完整评测
    python -m react_exp.eval_harness --suite gaia_l1

    # 运行回归子集 (快速门禁)
    python -m react_exp.eval_harness --suite gaia_l1 --subset regression

    # 对比两次运行
    python -m react_exp.eval_harness compare --baseline results/run_A.jsonl --current results/run_B.jsonl

    # 查看 failure mode 分布
    python -m react_exp.eval_harness analyze --input results/run_A.jsonl
"""
