...
async def chat(request: ChatRequest) -> ChatResponse:
    ...
    try:
        ...
        for item in result.get("action_trace", []):
            if item.node:
                _emit(trace_id=request_id, request_id=request_id,
                      session_id=request.session_id,
                      event_type=_TET.NODE_COMPLETED, title=item.node,
                      node_name=item.node,
                      event_status=item.status.value,
        ...
        for call in model_calls:
            payload = {
                ...
                "node_name": getattr(call, "node", ""),
                "purpose": getattr(call, "node", "") or "model_call",
            ...
            _emit(trace_id=request_id, request_id=request_id,
                  ...
                  title="模型调用",
                  node_name=getattr(call, "node", "") or "model",
